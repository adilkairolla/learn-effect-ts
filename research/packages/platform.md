...
- `HttpMiddleware` (`src/HttpMiddleware.ts`) — structural `HttpApp → HttpApp` transform. Built-in middlewares include `logger`, `cors`, `xForwardedHeaders`, `basicAuth`, and `serveStatic`; all are plain functions on `HttpApp` rather than framework plugins (`src/HttpMiddleware.ts:36-43`).
