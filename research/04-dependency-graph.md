...
- **`@effect/platform-node-shared` at depth 5, shallower than `@effect/platform-node` (depth 6).** This is counterintuitive by name, but `platform-node-shared` peers against `@effect/cluster`, `@effect/platform`, `@effect/rpc`, and `@effect/sql`. `platform-node` additionally takes `@effect/platform-node-shared` as a runtime `dependency`, pushing it one level deeper.
